...
type WalkthroughChecklistsPanelProps = { walkthroughId: string };

function statusLabel(value: string) {
  return value.replace(/_/g, " ");
}

export default function WalkthroughChecklistsPanel({ walkthroughId }: WalkthroughChecklistsPanelProps) {
  ...
  async function load() {
    ...
    const response = await fetch(`/api/walkthroughs/${walkthroughId}/checklists`, { cache: "no-store" });
  ...
  async function createChecklist(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch(`/api/walkthroughs/${walkthroughId}/checklists`, {
      method: "POST",
  ...
  return (
    <section className="rounded-[30px] border border-white/60 bg-white p-6 shadow-[0_20px_60px_rgba(15,23,42,0.06)]">
      <h3 className="text-xl font-semibold tracking-tight text-slate-950">Walkthrough Checklists</h3>
      ...
      <div className="mt-5 space-y-3">
        {isLoading ? <p className="text-sm text-slate-500">Loading walkthrough checklists...</p> : checklists.length === 0 ? <p className="text-sm text-slate-500">No walkthrough checklists yet.</p> : checklists.map((checklist) => (
